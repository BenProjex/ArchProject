...
class InstRegister():
   ...
   def Action(self, eprom):
      ...
      if cursor >= 2**12:
         ...
         currentByte = eprom.data[self.cursor]
         self.opCodes.append(currentByte)
         if(currentByte == 0x80):
            # Mov RX,RY
            fetchBytes(3)
            self.u113Sel.set_value(opCodes[2] / 0x10)
      ...
      self.instLen.set_value(len(opCodes))
      self.cursor = self.cursor + 1

   def fetchBytes(self, count):
      for i in range(1, count + 1):
         self.cursor = self.cursor + 1
         self.opCodes.append(eprom.data[self.cursor])
